...
const educations = [
  {
    icon: <FaBookOpen className="w-7 h-7 text-primary-400" />, 
  ...
  {
    icon: <FaMedal className="w-7 h-7 text-primary-400" />,
...
const EducationCertifications = () => (
  <section className="w-full py-16 px-2 sm:px-4 bg-[#0a1627] min-h-[80vh] flex items-center justify-center">
    <div className="max-w-5xl w-full mx-auto rounded-2xl bg-[#101c33] p-6 sm:p-10 shadow-2xl border border-primary-900/40">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Education */}
        <div>
          <div className="flex items-center gap-2 mb-6">
            <FaBookOpen className="w-6 h-6 text-primary-400" />
            <h2 className="text-2xl font-bold text-primary-200">Education</h2>
          </div>
              ...
              <div key={idx} className={`relative bg-gradient-to-br ${edu.border} p-1 rounded-2xl`}>
                <div className="flex items-center gap-4 bg-[#17213a] rounded-2xl p-5">
                  ...
                  <div>
                    ...
                    <div className="text-primary-200 text-sm font-medium mb-1">{edu.degree}</div>
                    <div className="text-primary-400 text-xs font-medium">{edu.years}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div>
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-2xl font-bold text-primary-200">Certificates</h2>
          </div>
          <div className="flex flex-col gap-6">
            {certificates.map((cert, idx) => (
              <div key={idx} className={`relative bg-[#17213a] rounded-2xl p-5 border ${cert.border} flex flex-col items-center justify-center min-h-[110px]`}> 
                ...
                <div className="mt-8 w-full flex flex-col items-center justify-center">
                  ...
                  {cert.org && <div className="text-primary-200 text-xs text-center mt-1">{cert.org}</div>}
                  <div className="text-primary-400 text-sm font-medium mt-1">{cert.year}</div>
                </div>
              </div>
            ))}
          </div>
