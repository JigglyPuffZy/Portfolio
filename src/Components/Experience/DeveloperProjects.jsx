...
const techIcons = {
  ...
  'Chatbot': <FaUserFriends className="text-primary-400" title="Chatbot" />,
  'AI Assistant': <FaUserFriends className="text-primary-400" title="AI Assistant" />,
  'Gemini API': <SiOpenai className="text-primary-500" title="Gemini API" />,
  'ChatGPT API': <SiOpenai className="text-green-500" title="ChatGPT API" />,
  'Flutter': <SiFlutter className="text-primary-400" title="Flutter" />,
  'Dart': <SiDart className="text-primary-500" title="Dart" />,
  'Cross-Platform Development': <SiFlutter className="text-primary-400" title="Cross-Platform Development" />,
  'QR Code System': <FaMobileAlt className="text-primary-400" title="QR Code System" />,
  'Real-time Data': <FaRocket className="text-primary-300" title="Real-time Data" />,
};
...
const mobileProjects = [
  ...
  {
    title: 'LaborLinkz',
    description: 'Cross-platform freelancing platform connecting primary-collar workers with employers, built with Flutter for seamless performance across devices.',
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center pt-28 pb-16 bg-white dark:bg-slate-900 font-sans overflow-x-hidden">
      ...
      <div className="pointer-events-none absolute inset-0 z-0">
        ...
        <div className="absolute top-10 left-1/3 w-72 h-72 bg-primary-100 dark:bg-primary-900 rounded-full blur-3xl opacity-20" />
      ...
      <div className="relative z-10 w-full max-w-6xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {mobileProjects.map((project, idx) => (
            <div
              key={idx}
              className="group bg-white/60 dark:bg-slate-800/60 backdrop-blur-lg rounded-3xl shadow-2xl border border-primary-100 dark:border-primary-700 flex flex-col overflow-hidden transform transition duration-300 hover:scale-[1.04] hover:shadow-primary-200 dark:hover:shadow-primary-900 hover:border-primary-400 hover:z-20 hover:-translate-y-2 hover:rotate-1 cursor-pointer relative before:absolute before:inset-0 before:rounded-3xl before:pointer-events-none before:transition before:duration-300 before:opacity-0 hover:before:opacity-100 before:shadow-[0_0_32px_8px_rgba(96,165,250,0.15)]"
            ...
            >
              ...
              <div className="p-6 flex-1 flex flex-col gap-2">
                <h3 className="text-xl font-bold mb-1 text-slate-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition duration-300">
                  {project.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-2 mt-auto items-center">
                  {project.tech.map((tech, i) => (
                    <span key={i} className="flex items-center gap-1 bg-primary-50/60 dark:bg-primary-900/60 text-primary-700 dark:text-primary-300 px-3 py-1 rounded-full text-sm font-semibold shadow-sm border border-primary-200 dark:border-primary-700 backdrop-blur-sm">
                      {techIcons[tech] || null}
                      {tech}
                    </span>
                  ))}
